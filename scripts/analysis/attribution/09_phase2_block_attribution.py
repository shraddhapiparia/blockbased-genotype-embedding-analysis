...
def load_model(ckpt_path: str, n_blocks: int, d_in: int,
               ...
               dropout: float = 0.0,
               block_dims: list = None) -> AttentionAggregator:
...
def run_lobo(
    ...
    for loop_idx, b in enumerate(indices_to_run):
        ...
        masked_emb = encode_batched(model, x_masked, batch_size)   # (N, d_model)
...
def run_noHLA_mode(args):
    ...
    if base_emb_saved.shape != (N, d_model):
        raise ValueError(
            f"Expected shape ({N}, {d_model}), got {base_emb_saved.shape}."
        )
    ...
    print(f"         AttentionAggregator (no-HLA): n_blocks={B}, d_in={d_in}, "
          f"d_model={d_model}, n_heads={n_heads}, n_layers={n_layers} — {n_params:,} params")
...
def main():
    ...
    print(f"         AttentionAggregator: n_blocks={B}, d_in={d_in}, d_model={d_model}, "
          f"n_heads={n_heads}, n_layers={n_layers} — {n_params:,} params")
